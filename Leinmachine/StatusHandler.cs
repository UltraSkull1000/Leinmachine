...
public class StatusHandler{ // Handles the statuses attached to the current user. 
    ...
    public async Task OnReady(){
        Leinmachine.Print($"Client is Ready, Preparing Status Handler...");
        _client.Ready -= OnReady; // Rmove listener, as it is no longer needed.
        await _client.SetCustomStatusAsync($"{Leinmachine.name} Successfully Started! {DateTime.Now.ToShortTimeString()}");
        ...
        timer.Start();
        Leinmachine.Print("Finished Preparing Status Handler.");
    }
}