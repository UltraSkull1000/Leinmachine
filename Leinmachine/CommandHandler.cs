...
using Leinmachine.Modules; // Classes under the folder ./modules

using Microsoft.Extensions.DependencyInjection; // ServiceProvider

using System.Reflection; // Assembly

namespace Leinmachine; 
public class CommandHandler // Handles interactions, mostly commands. 
{
    ...
    public CommandHandler(DiscordSocketClient _client) // Should only be created once.
    {
        ...
        Leinmachine.Print("Initialized Command Handler!"); // Finished Initialization
    }
    public async Task OnReady(){ // Client is ready to serve users.
        Leinmachine.Print($"Client is Ready, Preparing Services...");
        ...
        Leinmachine.Print("Finished Preparing Services.");
    }
    private async Task HandleInteraction(SocketInteraction interaction){
        ...
        {
            Leinmachine.Print(result.ErrorReason, ConsoleColor.Red);
        }
        else if(interaction.Type == InteractionType.ApplicationCommand) // Log that a command was executed
            Leinmachine.Print($"{interaction.User.Username} >> Executed Application Command", ConsoleColor.Blue);
    }
